...
public class ElasticScrollView extends ScrollView {
	...
	protected void readStyleAttributes(Context context, AttributeSet attrs,
			int defStyle) {

		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.elastic_scroll, 0, defStyle);
	...
	private void refreshOriginHeight(View view) {
		if (elasticView != null) {
			android.view.ViewGroup.LayoutParams layoutParams = elasticView
					.getLayoutParams();
	...
	private void refreshNormalRect() {
		...
		normalRect.set(mInnerView.getLeft(), mInnerView.getTop(),
				mInnerView.getRight(), mInnerView.getBottom());
	}

	private void moveInnerView(int deltaY) {
		mInnerView.layout(mInnerView.getLeft(), mInnerView.getTop() - deltaY,
				mInnerView.getRight(), mInnerView.getBottom() - deltaY);
	}
	...
	private void resetElasticView() {
		...
		ValueAnimator animator = ObjectAnimator.ofInt(
				elasticView.getLayoutParams().height, originHeight);
	...
	private void resetInnerView() {
		...
		animator.addUpdateListener(new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				int value = (Integer) animation.getAnimatedValue();
				mInnerView.layout(normalRect.left, normalRect.top + value,
						normalRect.right, normalRect.bottom + value);
			}
		});
		animator.start();
	...
	private boolean isNeedMove(int deltaY) {
		return deltaY == 0 ? false : (deltaY < 0 ? isNeedMoveTop()
				: isNeedMoveBottom());
	}
